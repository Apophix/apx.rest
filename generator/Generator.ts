	...
	public async generate(): Promise<void> {
		...
		for (const [schemaName, schema] of Object.entries<any>(components)) {
			...
			if (schema["enum"]) {
				...
				enumComponents.set(schemaName, new EnumComponent({ name: schemaName, values: schema["enum"] }));
			...
			if (requestNames.has(schemaName)) {
				...
				requestComponents.set(schemaName,
			...
			if (responseNames.has(schemaName)) {
				...
				responseComponents.set(schemaName,
		...
		for (const [endpoint, path] of Object.entries<any>(openApiDocument["paths"])) {
			for (const [method, operation] of Object.entries<any>(path)) {
				paths.push(new ApiPath({
					...
					requestComponentName: operation.requestBody?.content?.["application/json"]?.schema?.$ref?.split("/").pop(),
					responseComponentName: operation.responses["200"]?.content?.["application/json"]?.schema?.$ref?.split("/").pop(),
				}));
			}
		}
...
class ApiPath implements TApiPathDto {
	...
	public get clientMethodName(): string {
		if (this.responseComponent) {
			const lowerCaseComponentName = this.responseComponent.name.charAt(0).toLowerCase() + this.responseComponent.name.slice(1);
			return lowerCaseComponentName.replace("Response", "");
		}

		if (this.requestComponent) {
			const lowerCaseComponentName = this.requestComponent.name.charAt(0).toLowerCase() + this.requestComponent.name.slice(1);
			return lowerCaseComponentName.replace("Request", "");
		}

		return this.operationId;
	}

	private renderRequestAndResponse(requestDtoName: string, responseDtoName: string, finalResponse: string, clientFunctionName: string): string {
